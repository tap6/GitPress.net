...
export function EditorGitHistory({
  ...
  return (
    <section className="flex min-h-40 flex-1 flex-col overflow-hidden rounded border border-neutral-200 bg-white shadow-sm lg:min-h-0">
      <div className="flex shrink-0 items-baseline justify-between gap-2 border-b border-neutral-100 px-3 py-1.5">
        <h2 className="text-xs font-semibold text-neutral-700">Git 记录</h2>
        <Link
          href={`/sites/${siteId}/history`}
          className="text-[10px] leading-none text-neutral-400 hover:text-wp-accent hover:underline"
        >
          全部
        </Link>
      </div>
      <div className="min-h-0 flex-1 overflow-y-auto px-3 py-1.5">
        {error ? (
          <p className="text-[11px] leading-snug text-amber-700">{error}</p>
        ) : !hasFile ? (
          <p className="text-[11px] leading-snug text-neutral-400">
            保存到仓库后，这篇的改动会出现在这里。
          </p>
        ) : commits.length === 0 ? (
          <p className="text-[11px] leading-snug text-neutral-400">这篇还没有单独的提交记录。</p>
        ) : (
          <ol className="space-y-2">
            {commits.map((commit, index) => {
              ...
              return (
                <li key={commit.sha}>
                  {showDay ? (
                    <p className="mb-1 text-[10px] font-medium leading-none text-neutral-400">
                      {day}
                    </p>
                  ) : null}
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="block rounded px-0.5 py-0.5 hover:bg-neutral-50"
                  >
                    <p className="text-[11px] leading-snug text-neutral-700">{change.label}</p>
