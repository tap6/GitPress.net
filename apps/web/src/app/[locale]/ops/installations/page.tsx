...
export default async function OpsInstallationsPage() {
  ...
  return (
    <div className="mx-auto max-w-6xl">
      <h1 className="text-2xl font-semibold text-slate-900">{t("installsTitle")}</h1>
      <p className="mt-1 text-sm text-slate-500">{t("installsLead")}</p>
        ...
        <table className="w-full min-w-[720px] text-left text-sm">
          <thead className="border-b border-slate-100 text-xs uppercase tracking-wide text-slate-500">
            <tr>
              ...
              <th className="px-4 py-2 font-medium">{t("colInstall")}</th>
              <th className="px-4 py-2 font-medium">{t("colSites")}</th>
              <th className="px-4 py-2 font-medium">{t("colConnected")}</th>
            </tr>
          </thead>
          <tbody>
            {rows.length === 0 ? (
